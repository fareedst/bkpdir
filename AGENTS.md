...
- **TIED data access (MCP-first)**
  ...
  - **Single-writer enforcement**: Once MCP availability is confirmed at session start, the agent must **not** use IDE `Write`, `apply_patch`, or `StrReplace` on any **project-owned** YAML under `tied/` (indexes, `requirements/` / `architecture-decisions/` / `implementation-decisions/` details, `semantic-tokens.yaml`, `feedback.yaml`, excluding `tied/methodology/**`) when an MCP tool exists for that operation. Any bypass must be logged as a one-line exception (`TIED_YAML_BYPASS: <reason>`) in the step output. This is **not** advisory; it is a hard gate (see `docs/yaml-update-mcp-runbook.md` and checklist invariant `TIED_YAML_SINGLE_WRITER` in `docs/agent-req-implementation-checklist.yaml`).
...
### 3.3 During Work
- [ ] **Existence gate** ([PROC-AGENT_REQ_CHECKLIST] invariant `EXISTENCE_GATE`): Before `Read`/`Grep` on paths that may not exist (detail files not yet created, referenced docs, CITDP records), branch—search for the correct location, defer until the checklist step that creates the file, or record the absent reference explicitly; do not treat repeated File not found as ignorable noise.

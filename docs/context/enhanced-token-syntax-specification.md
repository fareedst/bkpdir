...
> **[HIGH] DOC-014: Enhanced implementation token system specification [DECISION: developer-experience, enables-flexibility, backward-compatible]**
...
## [ACTION:validation] Enhanced Token Format Specification
...
- **[CRITICAL] CRITICAL**: Core functionality, blocks other work
- **[HIGH] HIGH**: Important features, significant implementations
- **[MEDIUM] MEDIUM**: Conditional features, optional enhancements
- **[LOW] LOW**: Cleanup, maintenance, backward compatibility
...
- **[ACTION:discovery] SEARCH/DISCOVER**: File system discovery, configuration search, pattern matching
- **[ACTION:format-processing] DOCUMENT/UPDATE**: File writing, log creation, status updates, output formatting
- **[ACTION:core-functionality] CONFIGURE/MODIFY**: System setup, parameter adjustment, environment preparation
- **[ACTION:validation] PROTECT/VALIDATE**: Input validation, security checks, integrity verification
...
// [CRITICAL] ARCH-002: Atomic archive creation [DECISION: core-functionality, performance-critical, backward-compatible]
```
...
// [MEDIUM] DOC-012: Real-time validation feedback [DECISION: developer-experience, enables-flexibility]
```
...
// [LOW] COV-001: Coverage exclusion system [DECISION: infrastructure, independent]
```
...
// [HIGH] REFACTOR-003: Configuration abstraction [DECISION: enhancement, blocks-extraction]
```
...
```go
// [HIGH] CFG-005: Configuration inheritance [DECISION: enhancement, enables-flexibility, backward-compatible]
// [MEDIUM] DOC-012: Real-time validation [DECISION: developer-experience, enables-flexibility]
```
...
// [HIGH] REFACTOR-004: Error standardization [DECISION: enhancement, prerequisite-for-extraction]
```
...
// [LOW] GIT-004: Submodule support [DECISION: enhancement, independent]
```
...
// [HIGH] OUT-001: Delayed output [DECISION: enhancement, user-facing, backward-compatible]
```
...
```go
// [HIGH] CFG-005: Configuration inheritance [DECISION: enhancement, enables-flexibility, backward-compatible]
// [MEDIUM] DOC-007: Source code icons [DECISION: developer-experience, backward-compatible]
```
...
**Examples**:
```go
// [CRITICAL] FUTURE-API-001: New CLI structure [DECISION: core-functionality, breaking-change, user-facing]
// [HIGH] CFG-MIGRATION-001: Config format v2 [DECISION: enhancement, breaking-change]
```
...
**Examples**:
```go
// [CRITICAL] ARCH-002: Archive compression [DECISION: core-functionality, performance-critical, user-facing]
// [HIGH] FILE-003: File comparison [DECISION: enhancement, performance-critical, independent]
```

## [ACTION:format-processing] Usage Guidelines
...
```go
// [MEDIUM] SIMPLE-001: Basic utility function [DECISION: developer-experience]
```

#### **Two Category Selection** (Most Common):
```go
// [HIGH] CFG-005: Configuration inheritance [DECISION: enhancement, backward-compatible]
// [CRITICAL] ARCH-001: Archive naming [DECISION: core-functionality, user-facing]
```

#### **Three Category Selection** (Complex Implementations):
```go
// [CRITICAL] ARCH-001: Archive naming convention [DECISION: core-functionality, blocks-extraction, user-facing]
// [HIGH] REFACTOR-003: Config abstraction [DECISION: enhancement, blocks-extraction, backward-compatible]
```
...
2. **Priority Level**: [CRITICAL] typically `core-functionality`, [LOW] typically `infrastructure`
...
## [ACTION:core-functionality] Integration with Existing Systems
...
### **[CRITICAL] CRITICAL Priority Examples**
```go
// [CRITICAL] ARCH-001: Archive naming convention [DECISION: core-functionality, blocks-extraction, user-facing]
func GenerateArchiveName(cfg *Config, timestamp time.Time) string {
    // [CRITICAL] ARCH-001: Essential naming logic for user-facing archive files
...
func CreateArchiveWithCleanup(ctx context.Context) error {
    // [CRITICAL] ARCH-002: Critical resource management for atomic operations
...
### **[HIGH] HIGH Priority Examples**
```go
// [HIGH] CFG-005: Configuration inheritance system [DECISION: enhancement, enables-flexibility, backward-compatible]
func LoadConfigWithInheritance(configPath string) (*Config, error) {
    // [HIGH] CFG-005: Enhanced configuration loading with inheritance support
    return loadConfigRecursive(configPath, make(map[string]bool))
}

// [HIGH] GIT-001: Git information extraction [DECISION: enhancement, independent, user-facing]
func ExtractGitInfo(repoPath string) (*GitInfo, error) {
    // [HIGH] GIT-001: Git repository analysis for archive naming
    return analyzeGitRepository(repoPath)
}
```

### **[MEDIUM] MEDIUM Priority Examples**
```go
// [MEDIUM] DOC-010: Token format suggestions [DECISION: developer-experience, independent]
func SuggestTokenFormat(functionName string) (*TokenSuggestion, error) {
    // [MEDIUM] DOC-010: AI assistant guidance for token creation
    return analyzeFunction(functionName)
}

// [MEDIUM] TEST-002: Tools directory testing [DECISION: infrastructure, prerequisite-for-coverage]
func TestCoverageAnalysis(t *testing.T) {
    // [MEDIUM] TEST-002: Coverage validation for quality assurance tools
    validateCoverageTools()
}
```

### **[LOW] LOW Priority Examples**
```go
// [LOW] REFACTOR-001: Legacy compatibility wrapper [DECISION: infrastructure, backward-compatible]
...
// [LOW] COV-001: Coverage exclusion patterns [DECISION: infrastructure, independent]
func ApplyExclusionPatterns(files []string) []string {
    // [LOW] COV-001: Filter legacy code from coverage metrics

...
**Decision**: Migrate all semantic token definitions from `project-tokens.yaml` into `tied/semantic-tokens.md` while preserving the requirements → architecture → implementation → tests → code chain for every identifier.
...
- **Registry Layer**: Extend `tied/semantic-tokens.md` with tables that capture description, governance status, linked documents, and validation owners per token.
...
- A data-driven pipeline (sourced from existing `tied/*.md` plus representative code/test anchors) keeps token prominence accurate as the registry evolves.
...
- **Data Extraction Module**: Parse `tied/semantic-tokens.md`, `requirements.md`, `architecture-decisions.md`, `implementation-decisions.md`, and sampled code/test anchors to build a token graph (nodes: REQ/ARCH/IMPL/TEST/CODE; edges: traceability links) with token metadata (type, status, sample refs) for visual prominence.

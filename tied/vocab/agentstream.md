...
**Traceability:** `REQ-GOAGENT-LIB-MODULE` · `REQ-GOAGENT-CLI-CONFIG` · `REQ-GOAGENT-TEXT-SOURCES` · `REQ-GOAGENT-FEATURESPEC-BATCH` · `REQ-GOAGENT-YAML-STEP-RENDER` · `REQ-GOAGENT-PIPELINE-CHAIN` · `REQ-GOAGENT-AGENT-EXECUTOR` · `REQ-GOAGENT-NON-COMPACT-HTML-FORMAT` · `REQ-GOAGENT-CHECKLIST-CONTROL` · `ARCH-GOAGENT-*` · `IMPL-GOAGENT-*` (detail YAML in STDD source repository when absent from this client copy)

**Help coverage:** N/A (no in-app Help in this repository)

**STDD source note:** REQ/ARCH/IMPL detail files for `GOAGENT` tokens may live only in the TIED methodology repository; vocabulary terms here remain canonical for code under [`../../tools/agentstream/`](../../tools/agentstream/).
...
**Split/merge rule:** Go agentstream terms here; Ruby parity in [`agent-stream-ruby.md`](agent-stream-ruby.md); do not merge bkpdir product CLI terms.

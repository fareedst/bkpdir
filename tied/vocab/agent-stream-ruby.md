...
**Traceability:** `REQ-ATDD-COMPOS-AGENT_STREAM_TDD_YAML` · `REQ-ATDD-COMPOS-EXPORT_TDD_PROMPTS_STEPS` · `REQ-ATDD-E2E-AGENT_STREAM` · `ARCH-ATDD-*` · `IMPL-ATDD-*` (detail YAML in STDD source repository when absent from this client copy)

**Help coverage:** N/A (no in-app Help in this repository)

**STDD source note:** ATDD token detail files may live only in the TIED methodology repository; Ruby artifact paths in this repo remain authoritative for vocabulary.
...
**Split/merge rule:** Ruby runner terms here; Go preferred terms in [`agentstream.md`](agentstream.md).

...
**Decision**: Implement a deterministic migration workflow that relocates every legacy token entry from `project-tokens.yaml` into `tied/semantic-tokens.md`, generating STDD-compliant cross-links during the transfer.
...
2. **Cross-Link Synthesis**
   ...
   - Flag gaps by opening subtasks in `tied/tasks.md` whenever a requirement/decision/test link is missing.
...
4. **YAML Decommissioning**
   - After each category is represented in markdown, replace the YAML entries with a pointer stub until the file can be removed in a final cleanup change. `project-tokens.yaml` now contains a canonical pointer to `tied/semantic-tokens.md`.
5. **Validation Hooks**
...
**Migration Outcome:**
- All legacy tokens were migrated into `tied/semantic-tokens.md` and validated via `scripts/token-coverage-analysis.sh` and `scripts/validate-token-traceability.sh`.
...
.venv/bin/python scripts/render_token_tables.py --group features >> tied/semantic-tokens.md
scripts/validate-token-traceability.sh
scripts/token-coverage-analysis.sh
```

**Code Markers**: `project-tokens.yaml` migration scripts, `tied/semantic-tokens.md` tables annotated with `[REQ:DOC_016]`, validation logs appended to docs/governance summaries.
...
1. Record the requirement token `[REQ:EXTRACT_008_INTERDEP_MAPPING]` in `tied/requirements.md` (done).
2. Record the architecture decision `[ARCH:EXTRACT_008_INTERDEP]` in `tied/architecture-decisions.md` (done).
...
**Validation:**
- Confirm tokens appear in `tied/semantic-tokens.md` and trace to entries in requirements, architecture, and implementation documents.
...
### Documentation:
- Capture this audit in `tied/semantic-tokens.md` by registering `[IMPL:TOKEN_COVERAGE_AUDIT]` with the contexts above.
...
- Input sources: `tied/semantic-tokens.md`, `tied/requirements.md`, `tied/architecture-decisions.md`, `tied/implementation-decisions.md`, and sampled code/test anchors (grep for `[REQ:*]`, `[ARCH:*]`, `[IMPL:*]`).

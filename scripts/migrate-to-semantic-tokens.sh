...
# [CRITICAL] TOKEN-001: Semantic token migration script - [ACTION:migration]
# Purpose: Migrate from Unicode icons to semantic tokens for AI-first development
...
BACKUP_DIR="${PROJECT_ROOT}/backup-semantic-tokens-$(date +%Y%m%d-%H%M%S)"
LOG_FILE="${PROJECT_ROOT}/semantic-token-migration.log"
...
echo -e "${BLUE}🚀 Starting Semantic Token Migration${NC}"
...
migrate_file() {
    ...
    # Replace priority icons with semantic tokens
    for icon in "${!ICON_TO_SEMANTIC[@]}"; do
        local semantic="${ICON_TO_SEMANTIC[$icon]}"
        local count=$(grep -c "${icon}" "${temp_file}" 2>/dev/null || echo "0")
        if [ "$count" -gt 0 ]; then
            sed -i.bak "s/${icon}/${semantic}/g" "${temp_file}"
            changes_made=$((changes_made + count))
            log_message "REPLACE" "Replaced ${count} instances of '${icon}' with '${semantic}' in ${file}"
        fi
    done
    
    # Replace action icons with semantic tokens
    for icon in "${!ACTION_TO_SEMANTIC[@]}"; do
        local semantic="${ACTION_TO_SEMANTIC[$icon]}"
        local count=$(grep -c "${icon}" "${temp_file}" 2>/dev/null || echo "0")
        if [ "$count" -gt 0 ]; then
            sed -i.bak "s/${icon}/${semantic}/g" "${temp_file}"
            changes_made=$((changes_made + count))
...
generate_report() {
    local report_file="${PROJECT_ROOT}/semantic-token-migration-report.md"
